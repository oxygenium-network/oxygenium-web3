...
import { DUST_AMOUNT, ONE_ALPH, ScriptSimulator } from '@oxygenium/web3'
...
describe('transact', function () {
  it('should use transact methods for ALPH', async function () {
    ...
    const depositTx0 = await instance.transact.deposit({ signer, attoOxmAmount: ONE_ALPH })
    ...
    expect(depositCalls0[0].approvedAttoOxmAmount).toBe(ONE_ALPH)
    expect(depositCalls0[0].approvedTokens).toBe(undefined)

    const depositTx1 = await instance.transact.deposit({ signer, attoOxmAmount: ONE_ALPH })
    ...
    expect(depositCalls1[0].approvedAttoOxmAmount).toBe(ONE_ALPH)
    ...
    const withdrawTx1 = await instance.transact.withdraw({ signer, attoOxmAmount: ONE_ALPH })
    ...
    const getTx1 = await instance.transact.getTotalALPH({ signer, attoOxmAmount: ONE_ALPH })
  ...
  it('should use transact methods for token', async function () {
    ...
    expect(getCalls0[0].approvedAttoOxmAmount).toBe(undefined)
    expect(getCalls0[0].approvedTokens).toBe(undefined)
    const getTx1 = await instance.transact.getTotalTokens({ signer, attoOxmAmount: ONE_ALPH })
    const getCalls1 = ScriptSimulator.extractContractCallsWithErrors(getTx1.unsignedTx)
    expect(getCalls1.length).toBe(1)
    expect(getCalls1[0].contractAddress).toBe(instance.address)
    expect(getCalls1[0].approvedAttoOxmAmount).toBe(undefined)
  ...
  it('should test multicall', async function () {
    ...
    expect(depositsCalls[0].approvedAttoOxmAmount).toBe(ONE_ALPH)
    expect(depositsCalls[0].approvedTokens).toBe(undefined)
    expect(depositsCalls[1].contractAddress).toBe(instance1.address)
    expect(depositsCalls[1].approvedAttoOxmAmount).toBe(ONE_ALPH)
