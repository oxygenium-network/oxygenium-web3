...
import { DUST_AMOUNT, ONE_OXM, ScriptSimulator } from '@oxygenium/web3'
import { getSigner, mintToken } from '@oxygenium/web3-test'
import { MultiDeposit, MultiWithdraw, Transact } from '../artifacts/ts'

describe('transact', function () {
  it('should use transact methods for OXM', async function () {
    const signer = await getSigner(ONE_OXM * 10n)
    ...
    const depositTx0 = await instance.transact.deposit({ signer, attoOxmAmount: ONE_OXM })
    ...
    expect(depositCalls0[0].approvedAttoOxmAmount).toBe(ONE_OXM)
    expect(depositCalls0[0].approvedTokens).toBe(undefined)

    const depositTx1 = await instance.transact.deposit({ signer, attoOxmAmount: ONE_OXM })
    expect((await instance.view.getTotalOXM()).returns).toBe(ONE_OXM * 2n)
    ...
    expect(depositCalls1[0].approvedAttoOxmAmount).toBe(ONE_OXM)
    expect(depositCalls1[0].approvedTokens).toBe(undefined)

    const withdrawTx0 = await instance.transact.withdraw({ signer })
    expect((await instance.view.getTotalOXM()).returns).toBe(ONE_OXM)
    ...
    expect(withdrawCalls0[0].approvedAttoOxmAmount).toBe(undefined)
    expect(withdrawCalls0[0].approvedTokens).toBe(undefined)

    // Approve assets for the contract even if it's not needed
    const withdrawTx1 = await instance.transact.withdraw({ signer, attoOxmAmount: ONE_OXM })
    expect((await instance.view.getTotalOXM()).returns).toBe(0n)
    ...
    expect(withdrawCalls1[0].approvedAttoOxmAmount).toBe(undefined)
    expect(withdrawCalls1[0].approvedTokens).toBe(undefined)

    const getTx0 = await instance.transact.getTotalOXM({ signer })
    ...
    expect(getCalls0[0].approvedAttoOxmAmount).toBe(undefined)
    expect(getCalls0[0].approvedTokens).toBe(undefined)
    const getTx1 = await instance.transact.getTotalOXM({ signer, attoOxmAmount: ONE_OXM })
    ...
    expect(getCalls1[0].approvedAttoOxmAmount).toBe(undefined)
  ...
  it('should use transact methods for token', async function () {
    ...
    expect(depositCalls0.length).toBe(1)
    expect(depositCalls0[0].contractAddress).toBe(instance.address)
    expect(depositCalls0[0].approvedAttoOxmAmount).toBe(undefined)
    ...
    expect(depositCalls1.length).toBe(1)
    expect(depositCalls1[0].contractAddress).toBe(instance.address)
    expect(depositCalls1[0].approvedAttoOxmAmount).toBe(undefined)
    ...
    expect(getCalls0[0].approvedAttoOxmAmount).toBe(undefined)
    expect(getCalls0[0].approvedTokens).toBe(undefined)
    const getTx1 = await instance.transact.getTotalTokens({ signer, attoOxmAmount: ONE_OXM })
    const getCalls1 = ScriptSimulator.extractContractCallsWithErrors(getTx1.unsignedTx)
    expect(getCalls1.length).toBe(1)
    expect(getCalls1[0].contractAddress).toBe(instance.address)
    expect(getCalls1[0].approvedAttoOxmAmount).toBe(undefined)
  ...
  it('should test multicall', async function () {
    ...
    expect(depositsCalls[3].approvedAttoOxmAmount).toBe(undefined)
    expect(JSON.stringify(depositsCalls[3].approvedTokens)).toBe(JSON.stringify([{ id: tokenId, amount: 10n ** 18n }]))

    expect((await instance0.view.getTotalOXM()).returns).toBe(ONE_OXM)
    expect((await instance1.view.getTotalOXM()).returns).toBe(ONE_OXM)
