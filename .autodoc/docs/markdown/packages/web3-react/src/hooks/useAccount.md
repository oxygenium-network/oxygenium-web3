...
If the current connector is not WalletConnect or Desktop Wallet, the hook retrieves the default Alephium wallet using the `getDefaultAlephiumWallet` function from the `@oxygenium-network/get-extension-wallet` library. It then checks if the current account is already connected and matches the current network, address group, and key type. If so, it returns without doing anything.
...
- This code requires the `@oxygenium-network/get-extension-wallet` and `@oxygenium-network/web3` packages, as well as the `react` library.
