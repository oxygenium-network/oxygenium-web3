...
```typescript
import { getOxygenium } from 'alephium-web3/getOxygenium'

const providerUrl = 'https://example.com/alephium'
const alephium = getOxygenium(providerUrl)
```
...
3. What other modules are being exported from this file?
   - This file is exporting modules named `types`, `knownProviders`, and `getOxygenium` from other files in the `alephium-web3` project.