...
function importRalphMap(contract: Contract): string {
  ...
  return `import { RalphMap } from '@oxygenium-network/web3'`
}
...
function genContractByCodeHash(outDir: string) {
  const source = `
    ...
    import { Contract, ContractFactory } from '@oxygenium-network/web3'
...
export async function genLoadDeployments(config: Configuration) {
  ...
  const source = `
    ...
    import { RunScriptResult, DeployContractExecutionResult, NetworkId } from '@oxygenium-network/web3'
...
function genStructTypes(outDir: string) {
  ...
  const sourceCode = `
    ${header}
    import { Address, HexString, Val, Struct } from '@oxygenium-network/web3'
